...
#let draw-points = (points, size, buff) => {
  points
...
#let draw-clustering = (points, clusterings, size, buff) => {
  ...
  let draw-cluster = (indices, color) => {
  ...
  box(width: size, height: size, draw-points(points, size, buff) + clusterings.enumerate().map(indexed-clustering => place(draw-cluster(indexed-clustering.at(1), colors.at(calc.rem(indexed-clustering.at(0), colors.len()))))).sum())
}
